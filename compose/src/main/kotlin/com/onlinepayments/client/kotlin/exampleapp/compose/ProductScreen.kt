...
private fun PaymentProductItems(
    ...
    LazyColumn(
            ...
            items(basicPaymentItems.accountsOnFile) { accountOnFile ->
                val basicPaymentItem =
                    basicPaymentItems.basicPaymentItems.find {
                        it.getId() == accountOnFile.paymentProductId
                    }
                PaymentProductItem(
                    imageUrl = basicPaymentItem?.getDisplayHintsList()?.firstOrNull()?.logoUrl,
                    label = accountOnFile.displayHints?.labelTemplate?.getOrNull(0)?.mask?.let { mask ->
        ...
        items(basicPaymentItems.basicPaymentItems) { basicPaymentItem ->
            PaymentProductItem(
                imageUrl = assetsBaseUrl + basicPaymentItem.getDisplayHintsList()
                    .firstOrNull()?.logoUrl,
                label = basicPaymentItem.getDisplayHintsList().firstOrNull()?.label ?: "",
                onItemClicked = {
    ...
    when (selectedPaymentProduct) {
        ...
        is BasicPaymentProduct -> {
            when {
                ...
                selectedPaymentProduct.getId().equals(Constants.GOOGLE_PAY_PRODUCT_ID) -> {
                    launchGooglePay()
                }
