...
class PaymentProductAdapter(
            ...
            is AccountOnFile -> {
                val basicPaymentItem =
                    paymentProducts.find {
                        it is BasicPaymentItem && it.id == product.paymentProductId
                    } as? BasicPaymentItem
    ...
    inner class PaymentProductViewHolder(
        ...
        fun bindBasicPaymentItem(basicPaymentItem: BasicPaymentItem) {
            binding.root.run {
                Picasso.get()
                    .load(basicPaymentItem.displayHintsList[0].logoUrl)
                    .into(binding.ivPaymentProductLogo)
                binding.apply {
                    tvPaymentProductLabel.text = basicPaymentItem.displayHintsList[0].label
                    paymentProductItem.setOnClickListener { onBasicPaymentItemClicked.invoke(basicPaymentItem) }
                }
            }
        }
